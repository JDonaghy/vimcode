...
**Last updated:** Apr 27, 2026 (Session 336 — **Phase B.5 was a plumbing pass; runtime migration is B.5b, tracked at [#249](https://github.com/JDonaghy/vimcode/issues/249).** B.5 shipped 9 stages of trait surface + infrastructure on GTK (`2c8fe7f` … `2d8ef54`): `GtkBackend` struct, `Backend` trait impl, GDK→`UiEvent` translation helpers, accelerator registry, frame-scope, `is_modal_open()`, clipboard write, `open_url`. **What B.5 did NOT do** is migrate the actual GTK app runtime onto that infrastructure — only the quickfix panel routes through `Backend::draw_list`. Mouse/key dispatch still goes through Relm4 `Msg::*` flow; 22 inline `engine.<modal>.is_some()` gates still in place; 16 inline `matches_gtk_key` arms still drive keybindings; 24 direct `quadraui_gtk::draw_*` calls still bypass the trait. **B.5b** is the actual port: 13 sub-stages, ~5–8 sessions, fully detailed in #249 — pickup with `gh issue view 249`. The trait is *implemented* on two backends (TUI + GTK) but only TUI is *consuming* it at runtime today.)
