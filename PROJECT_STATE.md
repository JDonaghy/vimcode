...
**Last updated:** Apr 28, 2026 (Session 338 — **B.5b shipped end-to-end (Stages 1–13)** — [#249](https://github.com/JDonaghy/vimcode/issues/249) closed). Stages 1–7 wired the user-facing runtime migration: queue producers + drain, accelerator dispatch, modal-stack click routing for dialog / context-menu / completion / hover / tab-switcher. Stage 7 also added `Engine::is_blocking_modal_open()` as the single source of truth replacing 3 inline modal-enumeration sites. Stage 8 migrated 4 trait-callable `quadraui_gtk::draw_*` sites (palette / tree / form via `Backend::draw_*` + `enter_frame_scope`). Stage 9 extended the trait with `&Layout` parameters for the 5 layout-passthrough primitives (status_bar / tab_bar / activity_bar / terminal / text_display) per `BACKEND_TRAIT_PROPOSAL.md` §6.2; TUI + GTK trait impls updated. Stage 10: 3 of 5 GTK trait impls route through `quadraui::gtk::*`; the other 2 (activity_bar / terminal) stay as forward-compat stubs pending the #223 lift of `crate::gtk::quadraui_gtk::*` into quadraui itself (theme-type friction). Stage 11 dropped `App.modal_stack` / `App.drag_state` alias fields; ~30 call sites now reach via `self.backend.borrow().*_handle()`. Stage 12 deleted dead `draw_{tree, form, list, palette}` shims and removed `#![allow(dead_code)]` from `events.rs`. Smoke-test bugfixes this session: #247 / #250 / #251 / #252 all closed. 4078 lib + integration tests pass; clippy clean. **Next phase: B.6 — Win-GUI rebuild on quadraui.**
