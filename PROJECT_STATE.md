...
**Last updated:** Apr 27, 2026 (Session 337 — **B.5b Stage 1 shipped** ([#249](https://github.com/JDonaghy/vimcode/issues/249)). The editor DA's key/mouse-down/drag-update/drag-end/scroll callbacks now dual-write translated `UiEvent`s into `GtkBackend::events_handle()` alongside their existing `Msg::*` dispatch. A 16 ms `glib::timeout_add_local` drains the queue via `Backend::poll_events()`. Behavior unchanged — Relm4 `Msg` flow stays authoritative — but the foundation is in place for B5b.2 (accelerator dispatch off the queue). 4078 lib + integration tests pass; clippy clean. Sidebar/panel callbacks land alongside their click migration in B5b.3–B5b.7. Pickup: `gh issue view 249`.
