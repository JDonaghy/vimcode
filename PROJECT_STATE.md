...
**Last updated:** Apr 27, 2026 (Session 337 — **B.5b Stages 1–4 shipped** ([#249](https://github.com/JDonaghy/vimcode/issues/249)) + #250 fix. Stage 1: editor DA's key/mouse/scroll callbacks dual-write `UiEvent`s into `GtkBackend::events_handle()`; a 16 ms `glib::timeout_add_local` drains the queue via `Backend::poll_events()`. Stage 2: `dispatch_gtk_panel_accelerator` helper + `match_keypress` replace 13 inline `matches_gtk_key` arms; `util::matches_gtk_key` deleted. Stage 3: dialog modal click via `ModalStack` + `dispatch_mouse_down`. Stage 4: context-menu click via `ModalStack` + `dispatch_mouse_down`; inner row hit-test still uses `resolve_context_menu_click` cell-unit math. #250 closed: editor click now calls `engine.clear_sidebar_focus()` instead of clearing only `dap_sidebar_has_focus`. 4078 lib + integration tests pass; clippy clean. Next: B5b.5 (completion popup). Pickup: `gh issue view 249`.
