...
**Last updated:** Apr 27, 2026 (Session 335 — Phase B.4 Stage 6 (accelerator registry consolidation, `b400ce0`) and Stage 5c (drag-state consolidation + `grab_offset`, `f8d394f`) shipped. 14 panel-key bindings + `terminal.toggle_maximize` flow through `UiEvent::Accelerator` instead of inline `matches_tui_key` arms. 5 of 6 TUI scrollbar drags (search, settings, debug-sidebar, terminal-scrollback, debug-output) share `quadraui::DragState::ScrollbarY`; new `grab_offset: f32` field preserves cursor position on the thumb during drag. Editor-window scrollbar (`ScrollDragState`) deferred to Stage 5d (issue #244). Smoke-test follow-ups filed: #242 #243 #245 #246. Stage 7 (cleanup + GTK/Win-GUI restore + `BACKEND.md`) remains.)
