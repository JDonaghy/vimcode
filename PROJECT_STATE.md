...
**Last updated:** Apr 29, 2026 (Session 340 — **#266 partially shipped** in `779f6e8`: rich_text_popup (TUI + GTK) and completions (TUI) rasterisers lifted into `quadraui::{tui,gtk}` with 5 new theme fields (`link_fg`, `completion_bg/fg/border/selected_bg`). GTK rich_text_popup now takes `ui_font_desc` as an explicit parameter; the lifted scrollbar constants (`RICH_TEXT_POPUP_SB_WIDTH`/`_INSET`) are re-exported from quadraui so paint and click hit-test agree by construction. find_replace deferred to **#271** (new) because it requires a primitive migration of `FrHitRegion` / `FindReplaceClickTarget` / `FR_PANEL_WIDTH` / `compute_find_replace_hit_regions` from `core::engine` to `quadraui::primitives::find_replace` plus icon-constant rework — materially bigger than the other two lifts. Smoke filed: **#272** (GTK go-to-definition link click in focused hover does nothing). quadraui: 261 tests pass; vimcode `--no-default-features` + clippy clean across feature combos.
...
**Next priority arc set 2026-04-29** (full detail in PLAN.md "🎯 NEXT FOCUS" section): **#270 (GTK runner) → #271 (find_replace primitive lift) → #267 (dialog dual-Pango) → B.6 (Win-GUI rebuild)**. #270 mirrors #269 (TUI runner) symmetrically with a `GtkBackend` lift parallel to #268 plus the multi-DrawingArea design. #271 + #267 close the remaining trait-less primitives. B.6 was always queued for "after the trait surface is solid."
