...
**Next priority arc set 2026-04-29** (full detail in PLAN.md "🎯 NEXT FOCUS" section): **B.6 (Win-GUI rebuild on quadraui)**. The trait surface is now solid across TUI + GTK; every primitive has lifted rasterisers and trait coverage. B.6 rebuilds `src/win_gui/` as a clean `Backend` impl consuming `quadraui::win_gui::draw_*` rasterisers — the original "next phase" target before the entire #266→#271 arc.
