...
**Last updated:** Apr 27, 2026 (Session 337 — **B.5b Stages 1–6 shipped** ([#249](https://github.com/JDonaghy/vimcode/issues/249)) + 4 smoke-test fixes (#247 #250 #251 #252 all closed). Stages 1–4 wired the queue + drain, accelerator dispatch, dialog/context-menu modal-stack click routing. Stage 5: completion popup on `ModalStack`. Stage 6: hover-trigger gate (closes #247) suppresses the LSP hover when any blocking modal is open. #251: context-menu inner row hit-test now uses `quadraui::ContextMenuLayout::hit_test`. #250: editor click clears all sidebar focus flags. #252: native scrollbar widgets hidden under modals. 4078 lib + integration tests pass; clippy clean. Next: B5b.7 (tab-switcher modal stack). Pickup: `gh issue view 249`.
