...
**Last updated:** Apr 27, 2026 (Session 337 — **B.5b Stages 1–5 shipped** ([#249](https://github.com/JDonaghy/vimcode/issues/249)) + 3 smoke-test fixes (#250 #251 #252 all closed). Stages 1–4 wired the queue + drain, accelerator dispatch, dialog/context-menu modal-stack click routing. Stage 5: completion popup on `ModalStack`; click inside dismisses + consumes (cursor doesn't move on popup), click outside dismisses + propagates. Stage-4 also migrated the inner row hit-test to `quadraui::ContextMenuLayout::hit_test` to fix #251 (off-by-one for items below separators). #250 closed by routing editor click through `engine.clear_sidebar_focus()`. #252 closed by hiding native scrollbar widgets when any modal popup is up. 4078 lib + integration tests pass; clippy clean. Next: B5b.6 (hover popup). Pickup: `gh issue view 249`.
