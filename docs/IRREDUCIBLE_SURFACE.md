...
|---|---|---|
| **The folder / workspace picker is TUI-only.** GTK opens a *native* `GtkFileChooser`, deferred through `PendingFileDialog` and run from `tick()`, so there is no GTK canvas surface to paint, hit-test or arbitrate. | 5 — `render.rs:2919` (keyboard), `render.rs:7242` (`FrameOp::FolderPicker`), `render.rs:21569` (mouse-arbitration test), `gtk/mod.rs:6498` (the no-op compose arm), `tui_main/mouse.rs:691` (input) | ✅ **Irreducible.** Converging it means *building* a GTK canvas picker — adding per-backend code to make two tables match. |
| **Frame metrics are px on GTK, cells on TUI.** `FrameMetrics` carries only `line_height`/`char_width` and answers one question: "is this reserved band at least one line tall". Rect math stays per backend because Cairo painter-order and ratatui cell coalescence differ intrinsically. | 1 — `render.rs:7162` | ✅ **Irreducible.** Already reduced to the minimum: a unit, not a geometry. |
...
| **Command-line text selection is TUI-only.** GTK has no `cmd_sel`/`cmd_dragging` state, no inverted-cell read-back, and paints its command line through `Surface::CommandLine`, which exposes no character-offset hit test. | 1 — `tui_main/mouse.rs:1620` | ❌ **Not irreducible — mislabelled.** See §2. |

## 2. One of the four is a supply gap wearing a verdict's clothes
...
live in a backend directory. The genuinely irreducible surface is the three facts in §1
plus roughly 250 lines of native calls — a few hundred lines, not tens of thousands.
