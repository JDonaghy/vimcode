//! PROBE VERSION of the Neovim conformance harness (analysis worktree only).
//! Extensions over develop: per-case nvim setup Lua, viewport height synced from
//! nvim, macro playback queue pumped after every key, PROBE_FILTER env var.
...
fn run_in_neovim(lines: &[&str], cursor_line_1: usize, cursor_col_1: usize, keys: &str, setup: &str) -> Option<NvimResult> {
    ...
    lua.push_str(setup);
    lua.push('\n');
    // Fixture setup must not be undoable: drop undo history around set_lines.
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        lua.push('"'); lua.push_str(&escaped); lua.push('"');
    }
    lua.push_str("})\n");
    lua.push_str("vim.o.undolevels = 1000\n");
    lua.push_str(&format!("vim.api.nvim_win_set_cursor(0, {{{}, {}}})\n", cursor_line_1, cursor_col_1.saturating_sub(1)));
    let escaped_keys = keys.replace('\\', "\\\\").replace('"', "\\\"");
    lua.push_str(&format!(
        "pcall(function() vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes(\"{}\", true, false, true), \"ntx\", false) end)\n",
        escaped_keys
    ));
    let result_path = std::env::temp_dir().join(format!("vimcode_nvim_probe_{}.json", std::process::id()));
    ...
    let script_path = std::env::temp_dir().join(format!("vimcode_nvim_probe_{}.lua", std::process::id()));
    {
    ...
    let output = std::process::Command::new("nvim")
        .arg("--headless").arg("-u").arg("NONE").arg("-i").arg("NONE")
        .arg("-l").arg(script_path.to_string_lossy().as_ref())
        .output().ok()?;
...
fn press_char(engine: &mut Engine, ch: char) { engine.handle_key(&ch.to_string(), Some(ch), false); pump(engine); }
fn press_special(engine: &mut Engine, name: &str) { engine.handle_key(name, None, false); pump(engine); }
fn press_ctrl(engine: &mut Engine, ch: char) { engine.handle_key(&ch.to_string(), Some(ch), true); pump(engine); }
...
fn send_keys(engine: &mut Engine, keys: &str) {
    ...
    while let Some(ch) = chars.next() {
        if ch == '<' {
            ...
            let starts_special = chars.peek().map(|&c| c.is_ascii_uppercase() || c == 'C').unwrap_or(false);
            if has_closing && starts_special {
                ...
                match name.as_str() {
                    ...
                    n if n.starts_with("C-") => { let c = n.chars().nth(2).unwrap(); press_ctrl(engine, c); }
...
fn run_in_vimcode(lines: &[&str], cursor_line_1: usize, cursor_col_1: usize, keys: &str, rows: usize) -> (String, usize, usize) {
    let text = lines.join("\n");
    ...
    // nvim_win_set_cursor scrolls the window to show the cursor; mirror that.
...
struct Case { label: &'static str, lines: &'static [&'static str], cursor_line: usize, cursor_col: usize, keys: &'static str, setup: &'static str }
const fn c(label: &'static str, lines: &'static [&'static str], cursor_line: usize, cursor_col: usize, keys: &'static str) -> Case {
    Case { label, lines, cursor_line, cursor_col, keys, setup: "" }
}
const fn cs(label: &'static str, lines: &'static [&'static str], cursor_line: usize, cursor_col: usize, keys: &'static str, setup: &'static str) -> Case {
    Case { label, lines, cursor_line, cursor_col, keys, setup }
}
...
const CASES_OP: &[Case] = &[
    ...
    c("op:cw last word of line", &["foo bar","baz"], 1, 5, "cwX<Esc>"),
    c("op:c3w spans lines", &["one two","three four"], 1, 5, "c3wX<Esc>"),
    ...
    cs("op:cc noautoindent", &["    foo"], 1, 6, ":set noai<CR>ccX<Esc>", "vim.o.autoindent=false"),
    ...
    c("op:d) sentence", &["Hello world. Goodbye now. End."], 1, 1, "d)"),
    c("op:d( sentence", &["Hello world. Goodbye now. End."], 1, 14, "d("),
    ...
    c("op:d% multiline braces", &["if (x) {","  a;","}","b"], 1, 8, "d%"),
    ...
    c("op:d/pat multiline", &["aaa","bbb","ccc"], 1, 2, "d/ccc<CR>"),
    c("op:d/pat/+1 linewise", &["aaa","bbb","ccc","ddd"], 1, 1, "d/bbb/+1<CR>"),
    c("op:d/pat to col1 exclusive rule", &["aaa","bbb","ccc"], 1, 1, "d/ccc<CR>"),
    ...
    c("op:p linewise cursor first nonblank", &["  a","b"], 1, 1, "yyjp"),
    c("op:P linewise cursor", &["  a","b"], 1, 1, "yyjP"),
    c("op:p charwise multiline", &["abc","def","ghi"], 1, 2, "vjy$p"),
    c("op:P charwise multiline", &["abc","def","ghi"], 1, 2, "vjyP"),
    ...
    c("op:J after period (nvim nojoinspaces)", &["end.","next"], 1, 1, "J"),
    cs("op:J after period (vim joinspaces)", &["end.","next"], 1, 1, "J", "vim.o.joinspaces=true"),
    ...
    c("op:>> cursor", &["  abc"], 1, 4, ">>"),
    cs("op:>> cursor sol", &["  abc"], 1, 4, ">>", "vim.o.startofline=true"),
    c("op:>> then .", &["a"], 1, 1, ">>."),
    c("op:V2>", &["a","b"], 1, 1, "V2>"),
    ...
    c("op:>>>> noet ts4", &["a"], 1, 1, ":set ts=4 noet<CR>>>>>"),
    c("op:>> existing tab noet", &["\ta"], 1, 1, ":set ts=4 noet<CR>>>"),
    c("op:<< mixed tab space", &["\t  a"], 1, 1, ":set ts=4 noet<CR><<"),
    c("op:=G braces", &["int f() {","int x;","if (x) {","y();","}","}"], 1, 1, "=G"),
    c("op:=ip flat", &["  a","      b","c"], 1, 1, "=ip"),
    c("op:== single", &["      b"], 1, 1, "=="),
    c("op:gqq tw20", &["one two three four five six seven eight"], 1, 1, ":set tw=20<CR>gqq"),
    c("op:gqip tw20", &["one two three four five six seven eight","nine ten eleven twelve"], 1, 1, ":set tw=20<CR>gqip"),
    c("op:gqj joins short", &["one two","three"], 1, 1, ":set tw=30<CR>gqj"),
    c("op:gwip cursor", &["one two three four five six seven eight"], 1, 5, ":set tw=20<CR>gwip"),
    c("op:gqq cursor", &["one two three four five six seven eight"], 1, 5, ":set tw=20<CR>gqq"),
    c("op:gqq tw0 no wrap", &["one two three four five six seven eight"], 1, 1, "gqq"),
    c("op:gqq indented", &["    one two three four five six"], 1, 1, ":set tw=20<CR>gqq"),
    c("op:Vgq", &["one two three four five six"], 1, 1, ":set tw=10<CR>Vgq"),
    c("op:!Gsort", &["b","a"], 1, 1, "!Gsort<CR>"),
    c("op:!!tr", &["abc"], 1, 1, "!!tr a-z A-Z<CR>"),
    c("op:o autoindent", &["    foo"], 1, 1, "obar<Esc>"),
    c("op:O autoindent", &["    foo"], 1, 1, "Obar<Esc>"),
    c("op:o esc removes indent", &["    foo","bar"], 1, 1, "o<Esc>"),
    c("op:o x CR esc no trailing ws", &["    foo"], 1, 1, "ox<CR><Esc>"),
    ...
    cs("op:o noai", &["    foo"], 1, 1, ":set noai<CR>ox<Esc>", "vim.o.autoindent=false"),
    ...
    c("op:d<C-v>j blockwise force", &["abcde","fghij"], 1, 2, "d<C-v>j"),
];
...
const CASES_DOT: &[Case] = &[
    ...
    c("dot:\"1p . . increments", &["a","b","c","d"], 1, 1, "dddddd\"1p.."),
    ...
    c("dot:vjd . charwise", &["a1","b2","c3","d4","e5"], 1, 1, "vjd."),
    c("dot:<C-v>jIx .", &["ab","ab","ab","ab"], 1, 1, "<C-v>jIx<Esc>jj."),
...
const CASES_UNDO: &[Case] = &[
    ...
    c("undo:insert with CR is one undo", &["ab"], 1, 1, "ihello<CR>world<Esc>u"),
    ...
    c("undo:2u after insert ×3", &["a"], 1, 1, "Ax<Esc>Ay<Esc>Az<Esc>2u"),
    c("undo:u after dd on last line cursor", &["a","b","c"], 3, 1, "ddu"),
    c("undo:C-r cursor", &["a","b","c"], 2, 1, "ddu<C-r>"),
    c("undo:undo insert then cursor col", &["hello"], 1, 3, "ixyz<Esc>u"),
    c("undo:u after :m", &["a","b","c"], 1, 1, ":m$<CR>u"),
    c("undo:u after :t", &["a","b"], 1, 1, ":t$<CR>u"),
    c("undo:u after :normal", &["a","b"], 1, 1, ":%normal Ax<CR>u"),
];

// ─────────────────────────── D. registers ───────────────────────────
const CASES_REG: &[Case] = &[
    c("reg:\"ayy \"ap", &["a","b"], 1, 1, "\"ayyj\"ap"),
    c("reg:\"ayw \"Ayw \"ap", &["foo bar"], 1, 1, "\"ayww\"Ayw$\"ap"),
    c("reg:\"Ayy linewise append", &["a","b"], 1, 1, "\"ayyj\"Ayy\"ap"),
    c("reg:\"Ayy onto charwise becomes linewise", &["foo","bar"], 1, 1, "\"aywj\"Ayy\"ap"),
    c("reg:dd \"1p", &["a","b","c"], 1, 1, "ddj\"1p"),
    c("reg:dd dd \"2p", &["a","b","c"], 1, 1, "dddd\"2p"),
    c("reg:dw goes to \"-", &["foo bar"], 1, 1, "dw$\"-p"),
    c("reg:dw does not touch \"1", &["foo bar","x"], 1, 1, "jddkdw\"1p"),
    c("reg:d/ goes to \"1", &["foo bar baz"], 1, 1, "d/baz<CR>$\"1p"),
    c("reg:d% goes to \"1", &["(ab) cd"], 1, 1, "d%$\"1p"),
    c("reg:dn goes to \"1", &["x ab x ab"], 1, 1, "/ab<CR>ggdn$\"1p"),
    c("reg:yy dd \"0p", &["a","b","c"], 1, 1, "yyjdd\"0p"),
    c("reg:\"_dd then p", &["a","b","c"], 1, 1, "yyj\"_ddp"),
    c("reg:\"add \"bdd \"ap \"bp", &["a","b","c"], 1, 1, "\"add\"bdd\"ap\"bp"),
    ...
    c("reg:i C-r a linewise", &["a","b"], 1, 1, "\"ayyjA<C-r>a<Esc>"),
    c("reg:\"_x then p", &["abc"], 1, 1, "yl\"_xp"),
    c("reg:dd yy \"1p unchanged by yank", &["a","b","c"], 1, 1, "ddyy\"1p"),
    c("reg:\"adw does not set \"-", &["foo bar"], 1, 1, "\"adw\"-p"),
    c("reg:\"ayw x \"ap", &["foo bar"], 1, 1, "\"aywx\"ap"),
    c("reg:\"add then \"1p", &["a","b","c"], 1, 1, "\"add\"1p"),
    c("reg:\"Add appends", &["a","b","c"], 1, 1, "\"add\"Add\"ap"),
    c("reg:\"ayy \"ap count in visual", &["a","b"], 1, 1, "\"ayyjV\"ap"),
    ...
    c("reg:\"1p shifted by dd in visual", &["a","b","c"], 1, 1, "Vd\"1p"),
    c("reg:\"a in :normal", &["a","b"], 1, 1, "\"ayy:normal \"ap<CR>"),
    c("reg:\"= expr", &["a"], 1, 1, "\"=1+1<CR>p"),
    c("reg:C-r = in insert", &["a"], 1, 1, "A<C-r>=2*3<CR><Esc>"),
    c("reg:\"% file name empty", &["a"], 1, 1, "\"%p"),
    c("reg:paste count charwise multiline", &["ab","cd"], 1, 1, "vjy2p"),
    c("reg:p from \"1 then u then \"2p", &["a","b","c"], 1, 1, "dddd\"1pu\"2p"),
];

// ─────────────────────────── E. macros ───────────────────────────
const CASES_MAC: &[Case] = &[
    c("mac:qaxjq @a", &["ab","cd","ef"], 1, 1, "qaxjq@a"),
    c("mac:qaA! j q 2@a", &["a","b","c","d"], 1, 1, "qaA!<Esc>jq2@a"),
    c("mac:@a @@", &["a","b","c","d"], 1, 1, "qaA!<Esc>jq@a@@"),
    c("mac:10@a stops at failure", &["a,b","c,d","e f","g,h"], 1, 1, "qa0f,xjq10@a"),
    c("mac:qA append", &["ab","cd","ef"], 1, 1, "qaxqqAjq@a"),
    c("mac:macro with insert", &["a","b"], 1, 1, "qaIfoo <Esc>jq@a"),
    c("mac:macro with :s", &["a a","a a"], 1, 1, "qa:s/a/b/<CR>jq@a"),
    c("mac:macro with search", &["x foo","y foo","z foo"], 1, 1, "qa/foo<CR>rXq@a"),
    c("mac:\"ap shows macro", &["ab"], 1, 1, "qaxq\"ap"),
    c("mac:recursive", &["a","b","c","d"], 1, 1, "qaqqaA!<Esc>j@aq@a"),
    c("mac:@a then .", &["abcd"], 1, 1, "qaxq@a."),
    c("mac:3@a", &["a","b","c","d"], 1, 1, "qaddq3@a"),
    c("mac:count inside", &["a b c d e"], 1, 1, "qa2dwq@a"),
    c("mac:macro with visual", &["ab","cd","ef"], 1, 1, "qavlUjq@a"),
    c("mac:macro with dot", &["abc","def"], 1, 1, "qax.jq@a"),
    c("mac:macro with undo", &["abc"], 1, 1, "qaxuq@a"),
    c("mac:macro ending in insert", &["a","b"], 1, 1, "qaAx<Esc>q@a"),
    c("mac:@@ after 2@a", &["a","b","c","d","e"], 1, 1, "qaA!<Esc>jq2@a@@"),
    c("mac:macro with ctrl-a", &["1","1","1"], 1, 1, "qa<C-a>jq2@a"),
    c("mac:macro yank paste", &["a","b"], 1, 1, "qayypq@a"),
    c("mac:\"ay then @a executes text", &["ix<Esc>","b"], 1, 1, "\"ay$j@a"),
    c("mac:macro with ci(", &["f(a)","g(b)"], 1, 1, "qaci(X<Esc>jq@a"),
    c("mac:q register letter uppercase Q", &["ab","cd"], 1, 1, "qQxq@Q"),
];
...
const CASES_MARK: &[Case] = &[
    ...
    c("mark:'' after search", &["a","b","c foo"], 1, 1, "/foo<CR>''"),
    c("mark:'.", &["a","b","c"], 2, 1, "xgg'."),
    c("mark:`.", &["abc","def"], 2, 2, "xgg`."),
    c("mark:`] after yank", &["abc def"], 1, 1, "wyiw0`]"),
    c("mark:`[ after p", &["a","b"], 1, 1, "yyjp`["),
    c("mark:'> after V", &["a","b","c","d"], 2, 1, "Vj<Esc>gg'>"),
    c("mark:`< after v", &["abc","def"], 1, 2, "vjl<Esc>gg`<"),
    c("mark:mark shifts after O", &["a","b","c"], 2, 1, "maggOx<Esc>'a"),
    c("mark:mark on deleted line", &["a","b","c"], 2, 1, "maddgg'a"),
    ...
    c("mark:'a after text insert above", &["a","b"], 2, 1, "maggOx<CR>y<Esc>'a"),
    c("mark:`` after ''", &["a","b","c"], 1, 1, "G''``"),
    c("mark:y'a cursor", &["a","b","c"], 3, 1, "maggy'a"),
    c("mark:'a then '' back", &["a","b","c","d"], 4, 1, "magg'a''"),
    ...
    c("jump:gg G C-o C-o C-i", &["a","b","c","d"], 2, 1, "ggG<C-o><C-o><C-i>"),
    c("jump:/foo C-o", &["a","b","foo"], 1, 1, "/foo<CR><C-o>"),
    c("jump:3G 5G C-o C-o", &["1","2","3","4","5","6"], 1, 1, "3G5G<C-o><C-o>"),
    c("jump:C-o after :5", &["1","2","3","4","5","6"], 1, 1, "3G:5<CR><C-o>"),
    c("jump:C-o then new jump then C-i", &["1","2","3","4","5","6"], 1, 1, "3G5G<C-o>2G<C-i>"),
    ...
    c("jump:C-o twice same line dedup", &["a","b","c","d","e"], 1, 1, "3G3G5G<C-o><C-o>"),
    ...
    c("jump:g; after 2 changes same line", &["abcdef"], 1, 1, "x$xgg0g;g;"),
    c("jump:3<C-o>", &["1","2","3","4","5","6"], 1, 1, "2G3G4G5G3<C-o>"),
    c("jump:C-o after ''", &["a","b","c","d"], 1, 1, "3G''<C-o>"),
];
...
const CASES_SEARCH: &[Case] = &[
    ...
    c("search:/ N wraps", &["foo bar","baz bar"], 1, 1, "/bar<CR>N"),
    c("search:? then n backward", &["bar","foo","bar"], 2, 1, "?bar<CR>n"),
    c("search:? then N forward", &["bar","foo","bar"], 2, 1, "?bar<CR>N"),
    ...
    c("search:* then n keeps boundaries", &["foo foobar foo"], 1, 1, "*n"),
    c("search:* then :s//", &["foo bar foo"], 1, 1, "*:%s//X/g<CR>"),
    ...
    c("search:/pat/+1 linewise", &["a","foo","b","c"], 1, 1, "/foo/+1<CR>"),
    c("search:/pat/-1", &["a","b","foo"], 1, 1, "/foo/-1<CR>"),
    c("search:/pat/e then n keeps offset", &["foo bar foo bar"], 1, 1, "/bar/e<CR>n"),
    ...
    c("search:scs upper", &["x FOO Foo foo"], 1, 1, ":set ic scs<CR>/Foo<CR>"),
    c("search:scs lower", &["x FOO Foo foo"], 1, 1, ":set ic scs<CR>/foo<CR>"),
    c("search:\\c", &["x FOO foo"], 1, 1, "/\\cfoo<CR>"),
    c("search:\\C with ic", &["x foo FOO"], 1, 1, ":set ic<CR>/\\CFOO<CR>"),
    c("search:* with ic scs", &["Foo foo Foo"], 1, 1, ":set ic scs<CR>*"),
    c("search:// repeat", &["foo x foo x foo"], 1, 1, "/foo<CR>//<CR>"),
    c("search:/<CR> repeat", &["foo x foo x foo"], 1, 1, "/foo<CR>/<CR>"),
    ...
    c("search:/pat/;/pat2/", &["a foo b bar"], 1, 1, "/foo/;/bar<CR>"),
    ...
    c("search:/o\\nb multiline", &["foo","bar"], 1, 1, "/o\\nb<CR>"),
    c("search:/ then :s//", &["foo bar foo"], 1, 1, "/foo<CR>:s//X/g<CR>"),
    c("search:\\zs", &["foobar"], 1, 1, "/foo\\zsbar<CR>"),
    c("search:\\ze", &["xbar foobar"], 1, 1, "/foo\\zebar<CR>"),
    c("search:/. literal dot", &["abc a.c"], 1, 1, "/a\\.c<CR>"),
    c("search:/ with * quantifier", &["ac abc abbc"], 1, 1, "/ab*c<CR>"),
    c("search:/ with \\s", &["a\tb c"], 1, 1, "/\\s<CR>"),
    c("search:\\V nomagic", &["a.c abc"], 1, 1, "/\\Va.c<CR>"),
    c("search:/\\%V? skip", &["abc"], 1, 1, "l"),
    c("search:? n then N", &["bar","foo","bar","bar"], 4, 1, "?bar<CR>nN"),
    c("search:/ then ? then n", &["bar","foo","bar","bar"], 1, 1, "/bar<CR>?bar<CR>n"),
    c("search:d/pat/+0? linewise", &["a","foo","b"], 1, 1, "d/foo/0<CR>"),
    c("search:/\\(foo\\)\\1", &["foo foofoo"], 1, 1, "/\\(foo\\)\\1<CR>"),
    c("search:/\\w\\+ from col1", &["foo bar"], 1, 1, "/\\w\\+<CR>"),
    c("search:/$ empty match", &["ab","cd"], 1, 1, "/$<CR>"),
    c("search:/^ empty match", &["ab","cd"], 1, 1, "/^<CR>"),
    c("search:/\\n at eol", &["ab","cd"], 1, 1, "/\\n<CR>"),
    c("search:/ upper V with ic", &["abc ABC"], 1, 1, ":set ic<CR>/ABC<CR>"),
    c("search:* on number", &["12 x 12"], 1, 1, "*"),
    c("search:* on word with underscore", &["a_b x a_b"], 1, 1, "*"),
    c("search:gd", &["int x = 1;","y = x;"], 2, 5, "gd"),
    c("search:gn selects", &["foo bar foo"], 1, 1, "/foo<CR>ggcgnX<Esc>"),
    c("search:cgn .", &["foo bar foo baz foo"], 1, 1, "/foo<CR>ggcgnX<Esc>.."),
...
const CASES_EX: &[Case] = &[
    ...
    c("sub:'a,'b", &["a","a","a","a"], 1, 1, "majjmbgg:'a,'bs/a/b/<CR>"),
    c("sub:'<,'> auto", &["a","a","a","a"], 2, 1, "Vj:s/a/b/<CR>"),
    c("sub:'<,'> explicit", &["a","a","a","a"], 2, 1, "Vj<Esc>:'<,'>s/a/b/<CR>"),
    c("sub:i flag", &["A a"], 1, 1, ":s/a/b/gi<CR>"),
    c("sub:I flag with ic", &["A a"], 1, 1, ":set ic<CR>:s/a/b/gI<CR>"),
    c("sub:ic applies", &["A a"], 1, 1, ":set ic<CR>:s/a/b/g<CR>"),
    c("sub:n flag", &["a a"], 1, 1, ":s/a/b/gn<CR>"),
    c("sub:e flag", &["a"], 1, 1, ":s/z/b/e<CR>"),
    c("sub:& flag", &["a a","a a"], 1, 1, ":s/a/b/g<CR>j:s/a/c/&<CR>"),
    c("sub:&&", &["a a","a a"], 1, 1, ":s/a/b/g<CR>j:&&<CR>"),
    c("sub:& cmd", &["a a","a a"], 1, 1, ":s/a/b/g<CR>j:&<CR>"),
    c("sub:backrefs", &["ab"], 1, 1, ":s/\\(a\\)\\(b\\)/\\2\\1/<CR>"),
    ...
    c("sub:~ prev replacement", &["a b"], 1, 1, ":s/a/x/<CR>:s/b/~y/<CR>"),
    ...
    c("sub:empty pattern last search", &["foo bar foo"], 1, 1, "/foo<CR>:s//X/g<CR>"),
    c("sub:count", &["a","a","a","a"], 1, 1, ":s/a/b/ 2<CR>"),
    c("sub:range + count", &["a","a","a","a"], 1, 1, ":2s/a/b/ 2<CR>"),
    c("sub:trailing ws", &["a  ","b "], 1, 1, ":%s/\\s\\+$//e<CR>"),
    ...
    c("sub:\\u\\1 swap words", &["foo bar"], 1, 1, ":s/\\(\\w\\+\\) \\(\\w\\+\\)/\\u\\2 \\u\\1/<CR>"),
    c("sub:%s cursor at end", &["a","b","a"], 1, 1, ":%s/a/x/<CR>"),
    ...
    c("sub:& with \\n in pattern", &["a","b","c"], 1, 1, ":%s/\\n//<CR>"),
    c("sub:\\r in middle then cursor", &["abc"], 1, 1, ":s/b/\\r/<CR>"),
    ...
    c("sub:whole line", &["hello world"], 1, 1, ":s/\\v(\\w+) (\\w+)/\\2 \\1/<CR>"),
    ...
    c("g:cursor after", &["a","b","a","c"], 1, 1, ":g/a/s/a/x/<CR>"),
    c("g:j", &["a","b","a","b"], 1, 1, ":g/a/j<CR>"),
    c("g:range", &["a","a","a"], 1, 1, ":2,3g/a/s/a/b/<CR>"),
    c("g:delimiter", &["a","b"], 1, 1, ":g#a#d<CR>"),
    c("g:normal dd", &["a","b","a","c"], 1, 1, ":g/a/normal dd<CR>"),
    c("g:+1d", &["a","x","a","y"], 1, 1, ":g/a/+1d<CR>"),
    c("g:s// reuse pattern", &["a","b","a"], 1, 1, ":g/a/s//x/<CR>"),
    c("g:normal with count", &["ab","cd"], 1, 1, ":g/./normal 2x<CR>"),
    c("g:copy to end reversed order", &["a","b"], 1, 1, ":g/./t.<CR>"),
    c("g:d with count", &["a","1","2","b","3","4"], 1, 1, ":g/[ab]/d 2<CR>"),
    c("g:normal @a", &["a","b","a"], 1, 1, "qaAx<Esc>qu:g/a/normal @a<CR>"),
    ...
    c("ex:sort mixed case", &["b","A","a","B"], 1, 1, ":sort<CR>"),
    c("ex:2,3sort", &["c","b","a"], 1, 1, ":2,3sort<CR>"),
    c("ex:sort /pat/", &["x2 b","x1 a"], 1, 1, ":sort /x\\d /<CR>"),
    c("ex:sort /pat/ r", &["b 2","a 1"], 1, 1, ":sort /\\d/ r<CR>"),
    c("ex:sort n non-numbers first", &["b","2","a","1"], 1, 1, ":sort n<CR>"),
    c("ex:sort cursor", &["c","a","b"], 3, 1, ":sort<CR>"),
    c("ex:retab", &["\ta"], 1, 1, ":set ts=4<CR>:retab<CR>"),
    c("ex:retab!", &["    a"], 1, 1, ":set noet ts=4<CR>:retab!<CR>"),
    ...
    c("ex:/a/,/b/d", &["x","a","y","b","z"], 1, 1, ":/a/,/b/d<CR>"),
    c("ex:.,/foo/d", &["a","b","foo","c"], 1, 1, ":.,/foo/d<CR>"),
    c("ex:%d", &["a","b"], 1, 1, ":%d<CR>"),
    c("ex:%j", &["a","b","c"], 1, 1, ":%j<CR>"),
    ...
    c("ex:%normal Ax", &["a","b"], 1, 1, ":%normal Ax<CR>"),
    c("ex:2,3normal I-", &["a","b","c"], 1, 1, ":2,3normal I-<CR>"),
    ...
    c("ex:cursor after :%normal", &["a","b","c"], 1, 1, ":%normal Ax<CR>"),
    c("ex:cursor after :g/d", &["a","b","a","c"], 1, 1, ":g/a/d<CR>"),
    ...
    c("ex:.,.+1d", &["a","b","c"], 1, 1, ":.,.+1d<CR>"),
    c("ex:'<,'>d after v", &["a","b","c"], 1, 1, "vj<Esc>:'<,'>d<CR>"),
    c("ex:*d after visual", &["a","b","c"], 1, 1, "Vj<Esc>:*d<CR>"),
    c("ex:g/pat/normal cgn? skip", &["a"], 1, 1, "l"),
    c("ex:s with c flag skipped", &["a"], 1, 1, "l"),
    c("ex:3 goes col firstnonblank", &["a","b","  c"], 1, 1, ":3<CR>"),
    c("ex:0", &["a","b","c"], 3, 1, ":0<CR>"),
    c("ex:%s then n", &["a","b","a"], 1, 1, ":%s/a/x/<CR>ggn"),
    c("ex:s sets last search for n", &["a","b","a"], 1, 1, ":s/a/x/<CR>n"),
    c("ex:s cursor col", &["xx a"], 1, 4, ":s/a/b/<CR>"),
    c("ex:%s/x/y/g with \\r cursor", &["a,b,c"], 1, 1, ":s/,/\\r/g<CR>"),
    c("ex:noh no effect", &["a"], 1, 1, "/a<CR>:noh<CR>"),
    c("ex:2>3? shift count", &["a","b","c","d"], 1, 1, ":2> 2<CR>"),
    c("ex:< 2", &["    a","    b","    c"], 1, 1, ":< 2<CR>"),
    c("ex:>>> 3 levels", &["a"], 1, 1, ":>>><CR>"),
    c("ex:j with range and count", &["a","b","c","d","e"], 1, 1, ":2j 3<CR>"),
    c("ex:d x then \"xP", &["a","b"], 1, 1, ":d x<CR>\"xP"),
    c("ex:m with 0 on first", &["a","b"], 1, 1, ":m0<CR>"),
    c("ex:m to self", &["a","b"], 2, 1, ":m2<CR>"),
    c("ex:t with range dest .", &["a","b","c"], 3, 1, ":1,2t.<CR>"),
    c("ex:s on visual block ranges", &["a a","a a","a a"], 1, 1, "<C-v>j:s/a/b/<CR>"),
];

// ─────────────────────────── I. insert mode keys ───────────────────────────
const CASES_INS: &[Case] = &[
    c("ins:C-w", &["ab"], 1, 1, "Afoo bar<C-w><Esc>"),
    c("ins:C-w at line start joins", &["ab","cd"], 2, 1, "i<C-w><Esc>"),
    c("ins:C-w over existing text", &["ab cd"], 1, 6, "A<C-w><Esc>"),
    c("ins:C-w punctuation", &["foo.bar"], 1, 8, "A<C-w><Esc>"),
    c("ins:C-w trailing ws then word", &["foo bar  "], 1, 1, "A<C-w><Esc>"),
    ...
    c("ins:BS over indent (nvim smarttab)", &["    a"], 1, 5, "i<BS><Esc>"),
    cs("ins:BS over indent (nosmarttab)", &["    a"], 1, 5, "i<BS><Esc>", "vim.o.smarttab=false"),
    c("ins:BS mid indent", &["      a"], 1, 4, "i<BS><Esc>"),
    c("ins:Tab at start (smarttab)", &["x"], 1, 1, ":set ts=8<CR>i<Tab><Esc>"),
    cs("ins:Tab at start (nosmarttab)", &["x"], 1, 1, ":set ts=8<CR>i<Tab><Esc>", "vim.o.smarttab=false"),
    c("ins:Tab mid line ts4", &["a"], 1, 1, "A<Tab>x<Esc>"),
    c("ins:Tab mid line ts8", &["a"], 1, 1, ":set ts=8<CR>A<Tab>x<Esc>"),
    ...
    c("ins:CR then Esc removes autoindent", &["    foo"], 1, 8, "A<CR><Esc>"),
    c("ins:CR CR keeps prev line empty", &["    foo"], 1, 8, "A<CR><CR>x<Esc>"),
    ...
    c("ins:Down Down col memory", &["abcdef","ab","abcdef"], 1, 5, "i<Down><Down>X<Esc>"),
    ...
    c("ins:C-n completion", &["foo","f"], 2, 1, "A<C-n><Esc>"),
    c("ins:C-p completion", &["foo","fob","f"], 3, 1, "A<C-p><Esc>"),
    c("ins:typing prefix then CR no completion", &["foo bar"], 1, 1, "ofo<CR>x<Esc>"),
    c("ins:typing prefix then Tab", &["foo bar"], 1, 1, "ofo<Tab>x<Esc>"),
    ...
    c("ins:BS at start of insert over prev text", &["abc"], 1, 3, "i<BS><BS><Esc>"),
    c("ins:C-w at start of insert", &["foo bar"], 1, 5, "i<C-w><Esc>"),
    c("ins:A then BS past insert start", &["ab"], 1, 1, "A<BS><BS><BS><Esc>"),
    c("ins:C-t then C-d", &["a"], 1, 1, "i<C-t><C-t><C-d><Esc>"),
    c("ins:C-v u00e9", &["a"], 1, 1, "i<C-v>u00e9<Esc>"),
    c("ins:insert Tab then BS (sts)", &["a"], 1, 1, "A<Tab><BS>x<Esc>"),
    c("ins:C-e beyond line", &["abc","d"], 1, 1, "A<C-e><Esc>"),
    c("ins:i with count and Esc cursor", &["abc"], 1, 2, "2ix<Esc>"),
    c("ins:A with count and CR", &["a"], 1, 1, "2Ax<CR><Esc>"),
    c("ins:C-r register linewise mid line", &["a","bc"], 1, 1, "yyjli<C-r>\"<Esc>"),
    c("ins:C-r with tab in register", &["a\tb","c"], 1, 1, "yyjA<C-r>\"<Esc>"),
    c("ins:C-w C-w", &["a b c"], 1, 1, "A<C-w><C-w><Esc>"),
    c("ins:autoindent with tabs noet", &["\tfoo"], 1, 1, ":set noet<CR>obar<Esc>"),
    c("ins:C-t noet", &["a"], 1, 1, ":set noet<CR>i<C-t><Esc>"),
    c("ins:C-k digraph skip", &["a"], 1, 1, "l"),
    c("ins:BS join with autoindent", &["a","    b"], 2, 5, "i<BS><BS><BS><BS><BS><Esc>"),
...
const CASES_VIS: &[Case] = &[
    ...
    c("vis:gv after Vjd", &["a","b","c","d","e"], 1, 1, "Vjdgvd"),
    ...
    c("vis:vip then ip extends", &["a","","b","","c"], 1, 1, "vipipd"),
    ...
    c("vis:vjgq", &["one two three four five six","seven"], 1, 1, ":set tw=10<CR>Vjgq"),
    c("vis:vj: shows range then s", &["a","a","a"], 1, 1, "vj:s/a/b/g<CR>"),
    c("vis:vjy \"0 then p", &["ab","cd"], 1, 1, "vjy\"0P"),
    c("vis:vjp charwise reg into multi", &["abc","def","x"], 1, 1, "ylvjp"),
    c("vis:vjd cursor", &["abc","def","ghi"], 1, 2, "vjd"),
    c("vis:V G d cursor", &["a","b","c"], 2, 1, "VGd"),
    c("vis:v iw at word end", &["foo bar"], 1, 3, "viwd"),
    c("vis:v aw on last word with leading space", &["foo bar"], 1, 7, "vawd"),
    ...
    c("vis:vip on last para no trailing", &["a","","b","c"], 4, 1, "vipd"),
    c("vis:v_O charwise same as o", &["abcdef"], 1, 3, "vllOhd"),
    c("vis:v then < count", &["        a"], 1, 1, "V2<"),
    c("vis:v mode Esc then cursor", &["abc","def"], 1, 1, "vj<Esc>"),
    c("vis:v with $ then j keeps eol", &["ab","abcd","abc"], 1, 1, "v$jd"),
    c("vis:v with $ then j then y p", &["ab","abcd","abc"], 1, 1, "v$jyGp"),
];
...
const CASES_VB: &[Case] = &[
    ...
    c("vb:ragged d", &["abcdef","ab","abcdef"], 1, 3, "<C-v>jjlld"),
    c("vb:I on short line skipped", &["abcdef","ab","abcdef"], 1, 4, "<C-v>jjIx<Esc>"),
    c("vb:A on short line padded", &["abcdef","ab","abcdef"], 1, 4, "<C-v>jjAx<Esc>"),
    ...
    c("vb:jIx then .", &["ab","ab","ab","ab"], 1, 1, "<C-v>jIx<Esc>jj."),
    c("vb:I on empty middle line", &["ab","","ab"], 1, 1, "<C-v>jjIx<Esc>"),
    c("vb:A on empty middle line", &["ab","","ab"], 1, 1, "<C-v>jjAx<Esc>"),
    c("vb:$A on empty middle line", &["ab","","ab"], 1, 1, "<C-v>jj$Ax<Esc>"),
    c("vb:jjy p at eol", &["ab","cd","ef"], 1, 1, "<C-v>jjy$p"),
    c("vb:jjy p on shorter", &["abc","abc","x"], 1, 2, "<C-v>jjyGp"),
    c("vb:jly then p", &["abc","def"], 1, 1, "<C-v>jly$p"),
    c("vb:jIx with CR", &["ab","ab"], 1, 1, "<C-v>jIx<CR><Esc>"),
    c("vb:jc with multi chars", &["abcd","abcd"], 1, 2, "<C-v>jlcXYZ<Esc>"),
    c("vb:j< ", &["    ab","    ab"], 1, 5, "<C-v>j<"),
    c("vb:jr<CR>", &["abc","def"], 1, 2, "<C-v>jr<CR>"),
    c("vb:cursor after d", &["abcdef","abcdef"], 1, 3, "<C-v>jld"),
    c("vb:cursor after y", &["abcdef","abcdef"], 2, 4, "<C-v>khhy"),
    c("vb:jjAx then u", &["ab","ab","ab"], 1, 1, "<C-v>jjAx<Esc>u"),
    c("vb:2j then o then j", &["abc","abc","abc","abc"], 1, 1, "<C-v>2jlojd"),
    c("vb:I with count? 2I", &["ab","ab"], 1, 1, "<C-v>j2Ix<Esc>"),
    c("vb:jjp block over block", &["ab","cd","ef","gh"], 1, 1, "<C-v>jy2j<C-v>jp"),
    c("vb:vb yank then p linewise reg? P", &["ab","cd"], 1, 1, "<C-v>jyP"),
    c("vb:$ then I", &["ab","abcd"], 1, 1, "<C-v>j$Ix<Esc>"),
    c("vb:d then .", &["abcd","abcd","abcd","abcd"], 1, 1, "<C-v>jdjj."),
    c("vb:r then .", &["abcd","abcd","abcd","abcd"], 1, 1, "<C-v>jlrxjj."),
    c("vb:c then .", &["abcd","abcd","abcd","abcd"], 1, 1, "<C-v>jcX<Esc>jj."),
    c("vb:jjIx on tab lines", &["\tab","\tab"], 1, 2, "<C-v>jIx<Esc>"),
...
const CASES_NUM: &[Case] = &[
    ...
    cs("num:octal nf=octal 007", &["007"], 1, 1, "<C-a>", "vim.o.nrformats='bin,octal,hex'"),
    ...
    cs("num:alpha", &["a"], 1, 1, "<C-a>", "vim.o.nrformats='alpha'"),
    c("num:C-a on 9 width", &["9"], 1, 1, "<C-a>"),
    c("num:C-a on 99999999999999999999 overflow", &["99999999999999999999"], 1, 1, "<C-a>"),
    c("num:V C-a skips lines without numbers", &["1","x","1"], 1, 1, "Vjjg<C-a>"),
    c("num:C-a on number after word char", &["ab12"], 1, 3, "<C-a>"),
    c("num:V C-a only first number per line", &["1 2","3 4"], 1, 1, "Vj<C-a>"),
    ...
    c("num:C-a cursor on space before number", &["a 1"], 1, 2, "<C-a>"),
    c("num:C-a on 10 then u", &["10"], 1, 1, "<C-a>u"),
    c("num:V C-a cursor", &["1","1"], 1, 1, "Vj<C-a>"),
    c("num:v C-a on -5 in visual (no minus)", &["x -5"], 1, 4, "vl<C-a>"),
];
...
const CASES_SCROLL: &[Case] = &[
    ...
    cs("scroll:C-d col sol", LONG, 1, 3, "<C-d>", "vim.o.startofline=true"),
    ...
    c("scroll:3C-d sets scroll then C-u", LONG, 30, 1, "3<C-d><C-u>"),
    c("scroll:3<C-f>", LONG, 1, 1, "3<C-f>"),
    c("scroll:2<C-b>", LONG, 60, 1, "2<C-b>"),
    c("scroll:z<CR> col first nonblank", &["a","   b","c"], 2, 1, "z<CR>"),
    c("scroll:zt col kept", &["a","   b","c"], 2, 1, "zt"),
    c("scroll:H on short buffer", &["a","b","c"], 3, 1, "H"),
    c("scroll:L on short buffer", &["a","b","c"], 1, 1, "L"),
    c("scroll:M on short buffer", &["a","b","c","d","e"], 1, 1, "M"),
    c("scroll:C-d on short buffer", &["a","b","c","d","e"], 1, 1, "<C-d>"),
    c("scroll:C-f on short buffer", &["a","b","c","d","e"], 1, 1, "<C-f>"),
    c("scroll:C-e on short buffer", &["a","b","c"], 1, 1, "<C-e>"),
    c("scroll:C-u on short buffer", &["a","b","c","d","e"], 5, 1, "<C-u>"),
    c("scroll:3<C-d> on short", &["a","b","c","d","e","f","g","h"], 1, 1, "3<C-d>"),
...
const CASES_WORD: &[Case] = &[
    ...
    c("word:w over multiple blank lines", &["a","","","b"], 1, 1, "w"),
    c("word:ww over multiple blank lines", &["a","","","b"], 1, 1, "ww"),
    c("word:w from whitespace-only line", &["a","   ","b"], 2, 1, "w"),
    c("word:e from whitespace-only line", &["a","   ","b"], 2, 1, "e"),
    c("word:) sentences", &["Hello world.  Second one.  Third."], 1, 1, ")"),
    c("word:)) sentences", &["Hello world.  Second one.  Third."], 1, 1, "))"),
    c("word:( sentence", &["Hello world.  Second one.  Third."], 1, 20, "("),
    c("word:) single space", &["Hello world. Second one."], 1, 1, ")"),
    ...
    c("word:} whitespace-only line not blank", &["a","   ","b",""], 1, 1, "}"),
    c("word:]]", &["{","a","}","{","b"], 1, 1, "]]"),
    c("word:[[", &["{","a","}","{","b"], 5, 1, "[["),
    c("word:[{", &["{","a","}"], 2, 1, "[{"),
    c("word:]}", &["{","a","}"], 2, 1, "]}"),
    ...
    c("word:jj col memory", &["abcdef","ab","abcdef"], 1, 5, "jj"),
    ...
    cs("word:gg indented (sol)", &["  a","b"], 2, 1, "gg", "vim.o.startofline=true"),
    c("word:G indented (nosol)", &["a","  b"], 1, 1, "G"),
    cs("word:G indented (sol)", &["a","  b"], 1, 1, "G", "vim.o.startofline=true"),
    ...
    c("word:$ then k then j", &["abcdef","ab","abcdef"], 2, 1, "$kj"),
    ...
    c("word:A esc then j col memory", &["ab","abcdef"], 1, 1, "A<Esc>j"),
    ...
    c("word:dd then j col? (nosol)", &["abcdef","abcdef","abcdef"], 1, 3, "ddj"),
    c("word:>> then j col", &["abcdef","abcdef"], 1, 3, ">>j"),
    c("word:u then j col", &["abcdef","abcdef"], 1, 3, "xuj"),
    c("word:: then j col", &["abcdef","abcdef"], 1, 3, ":noh<CR>j"),
    c("word:/ then j col", &["abcdef","abcdef"], 1, 1, "/c<CR>j"),
    c("word:zz then j col", &["abcdef","abcdef"], 1, 3, "zzj"),
    c("word:5G then j col (nosol)", &["abcdef","abcdef","abcdef"], 1, 3, "2Gj"),
    cs("word:5G then j col (sol)", &["abcdef","abcdef","abcdef"], 1, 3, "2Gj", "vim.o.startofline=true"),
    c("word:H then j col", &["abcdef","abcdef","abcdef"], 2, 3, "Hj"),
    ...
    c("word:w at eol with trailing space", &["ab ","cd"], 1, 2, "w"),
    c("word:b from col1 of indented line", &["ab","  cd"], 2, 3, "b"),
    c("word:b from start of indented line", &["ab","  cd"], 2, 1, "b"),
    c("word:e from indented line start", &["ab","  cd"], 2, 1, "e"),
    c("word:ge from indented line start", &["ab","  cd"], 2, 1, "ge"),
...
const CASES_TO: &[Case] = &[
    ...
    c("to:das last sentence", &["One two.  Three four."], 1, 12, "das"),
    c("to:das first sentence", &["One two.  Three four."], 1, 2, "das"),
    c("to:dis on whitespace between", &["One two.  Three four."], 1, 10, "dis"),
    c("to:dip", &["a","b","","c"], 1, 1, "dip"),
    c("to:dap", &["a","b","","c"], 1, 1, "dap"),
    c("to:dap trailing no blank", &["a","","b","c"], 3, 1, "dap"),
    ...
    c("to:ci{ multiline", &["{","  a","  b","}"], 2, 3, "ci{X<Esc>"),
    c("to:di{ same line", &["f {a}"], 1, 4, "di{"),
    c("to:di{ on line with brace and text", &["if (x) {","  a","}"], 2, 3, "di{"),
    c("to:yi{ cursor multiline", &["{","  a","  b","}"], 3, 3, "yi{"),
    c("to:di[", &["a[1]"], 1, 3, "di["),
    c("to:da[", &["a[1]"], 1, 3, "da["),
    ...
    c("to:di\" between two strings", &["\"a\" x \"b\""], 1, 5, "di\""),
    ...
    c("to:da\" with leading and trailing ws", &["a  \"b\"  c"], 1, 5, "da\""),
    ...
    c("to:dat self-closing inside", &["<a><br/>x</a>"], 1, 9, "dat"),
    ...
    c("to:di( with newline after open", &["f(","a","b)"], 2, 1, "di("),
    c("to:da{ with trailing text", &["x {a} y"], 1, 4, "da{"),
    c("to:diw at eol on space", &["foo "], 1, 4, "diw"),
    c("to:daw on multi spaces at start", &["   foo"], 1, 1, "daw"),
    c("to:dis multi-line sentence", &["One two","three.  Four."], 1, 1, "dis"),
    c("to:das multi-line", &["One two","three.  Four."], 1, 1, "das"),
    c("to:dip with indented lines", &["  a","  b","","c"], 1, 1, "dip"),
    ...
    c("to:di\" cursor on quote when 3 quotes", &["a \"b\" c \" d"], 1, 8, "di\""),
    ...
    c("to:dip cursor after", &["a","","b","c","","d"], 3, 1, "dip"),
    c("to:dap cursor after", &["a","","b","c","","d"], 3, 1, "dap"),
];
...
const CASES_MISC: &[Case] = &[
    ...
    c("misc:p multi-line charwise cursor", &["ab","cd"], 1, 1, "vjy$p"),
    c("misc:gp charwise multi", &["ab","cd"], 1, 1, "vjy$gp"),
    c("misc:gP charwise", &["abc"], 1, 2, "ylgP"),
    c("misc:2gp", &["a","b"], 1, 1, "yy2gp"),
    c("misc:p with count linewise cursor", &["a","b"], 1, 1, "yy3p"),
    ...
    c("misc:R at eol then BS", &["ab"], 1, 2, "Rxyz<BS><BS><BS><BS><Esc>"),
    ...
    c("misc:& after &&", &["a a a","a a a"], 1, 1, ":s/a/b/g<CR>j&"),
    c("misc:g& after range", &["a","a","a"], 1, 1, ":1s/a/b/<CR>g&"),
    ...
    c("misc:count then : then range", &["a","b","c"], 1, 1, "2:normal Ax<CR>"),
    ...
    c("misc:g?g?", &["ab"], 1, 1, "g?g?"),
    c("misc:gqgq", &["one two three four five six"], 1, 1, ":set tw=10<CR>gqgq"),
    c("misc:gwgw", &["one two three four five six"], 1, 8, ":set tw=10<CR>gwgw"),
    c("misc:d then d with register", &["a","b"], 1, 1, "\"add\"ap"),
    c("misc:ZZ skip", &["a"], 1, 1, "l"),
    c("misc:. after :normal", &["ab","cd"], 1, 1, ":normal x<CR>j."),
    c("misc:. after :g normal", &["ab","cd","ef"], 1, 1, ":1,2g/./normal x<CR>G."),
    c("misc:xp then . ", &["abcd"], 1, 1, "xp."),
    c("misc:dot after @: ", &["a","b","c","d"], 1, 1, ":d<CR>@:."),
    c("misc:tilde op setting? tildeop skip", &["ab"], 1, 1, "l"),
    c("misc:count with text object c", &["a b c d"], 1, 1, "3ciwX<Esc>"),
    c("misc:count before and after with textobj", &["a b c d e f g"], 1, 1, "2d2aw"),
    ...
    c("misc:cc on last line indent", &["a","  b"], 2, 3, "ccX<Esc>"),
...
const ALL: &[&[Case]] = &[
    CASES_OP, CASES_DOT, CASES_UNDO, CASES_REG, CASES_MAC, CASES_MARK, CASES_SEARCH,
    CASES_EX, CASES_INS, CASES_VIS, CASES_VB, CASES_NUM, CASES_SCROLL, CASES_WORD, CASES_TO, CASES_MISC,
];

#[test]
fn nvim_conformance() {
    let nvim_ok = std::process::Command::new("nvim").arg("--version").output().map(|o| o.status.success()).unwrap_or(false);
    if !nvim_ok { eprintln!("SKIP: nvim not found on PATH"); return; }
    let filter = std::env::var("PROBE_FILTER").ok();
    let mut passed = 0;
    let mut failed = 0;
    let mut failures = Vec::new();
    for case in ALL.iter().flat_map(|g| g.iter()) {
        if let Some(f) = &filter { if !case.label.contains(f.as_str()) { continue; } }
        let nvim = match run_in_neovim(case.lines, case.cursor_line, case.cursor_col, case.keys, case.setup) {
            Some(r) => r,
            None => { println!("  SKIP [{}]: nvim execution failed", case.label); continue; }
        };
        let (vc_buf, vc_line, vc_col) = run_in_vimcode(case.lines, case.cursor_line, case.cursor_col, case.keys, nvim.rows);
        ...
        let cursor_match = if vc_line == nvim.line && vc_col == nvim.col {
            true
        } else if vc_buf.ends_with('\n') && nvim.buf.last().map(|s| s.is_empty()).unwrap_or(false) && vc_line + 1 == nvim.line && vc_col == nvim.col {
            true
        } else { false };
        if buf_match && cursor_match {
            passed += 1;
            if std::env::var("PROBE_VERBOSE").is_ok() { println!("PASS [{}]", case.label); }
        } else {
            failed += 1;
            let what = match (buf_match, cursor_match) { (false, false) => "BUF+CUR", (false, true) => "BUF", _ => "CUR" };
            failures.push(format!(
                "FAIL {} [{}] keys={:?} start={:?}@({},{})\n  buffer: nvim={:?} vimcode={:?}\n  cursor: nvim=({},{}) vimcode=({},{})\n",
                what, case.label, case.keys, case.lines, case.cursor_line, case.cursor_col, nvim_buf, vc_buf.trim_end_matches('\n'), nvim.line, nvim.col, vc_line, vc_col
            ));
        }
    }
