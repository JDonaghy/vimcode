...
#   - session/prompt     -> emits a session/update notification, then a
#                           scripted agent->client request (fixed id 9001,
#                           method fs/read_text_file), then BLOCKS reading
#                           one more line before replying with
#                           stopReason: end_turn. This proves a reply
#                           written out of band via AcpClient::
#                           respond_to_client_request actually reaches the
#                           agent and unblocks it — not just that the
#                           client-side bookkeeping looks right.
...
while IFS= read -r line; do
  case "$line" in
    ...
    *'"method":"session/prompt"'*)
      id=$(extract_id "$line")
      printf '{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sess-1","update":{"kind":"agent_message_chunk","text":"thinking..."}}}\n'
