...
pub struct Settings {
    ...
    /// User-configured MCP servers (#1462) sent on every `session/new` —
    /// the global list, shared across all `acp_agents` entries (and the
    /// single-agent `acp_agent_command` path) unless an entry's own
    /// `AcpAgentProfile::mcp_servers` overrides it (see that field's doc,
    /// and `Engine::acp_resolve_mcp_servers`). Shape mirrors ACP's
    /// `McpServer` wire type — `transport` picks `"stdio"` (default),
    /// `"http"`, or `"sse"`; `command`/`args`/`env` apply to `stdio`,
    /// `url`/`headers` to `http`/`sse`. An `http`/`sse` entry is dropped
    /// at `session/new` time (with a status-line warning) if the live
    /// agent's `initialize` response doesn't advertise
    /// `agentCapabilities.mcpCapabilities.http`/`.sse` — `stdio` is never
    /// dropped, it's baseline ACP v1.
    ///
    /// Empty (the default, every pre-#1462 config) sends no MCP servers
    /// at all — unchanged from before this setting existed.
    ///
