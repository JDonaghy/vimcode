...
pub struct AcpAgentProfile {
    ...
    #[serde(default)]
    pub env: Vec<String>,
    /// Per-agent override of `settings.acp_mcp_servers` (#1462): any entry
    /// here whose `name` matches a global entry replaces it; any other
    /// entry is appended. Empty (the default, every pre-#1462 profile)
...
// ---------------------------------------------------------------------------
// User-configured MCP servers (#1462)
// ---------------------------------------------------------------------------
...
/// for `session/new`'s `mcpServers` field (#1462), dropping any `http`/
/// `sse` entry the live agent hasn't advertised support for via `caps`
/// (`agentCapabilities.mcpCapabilities`, see [`parse_mcp_capabilities`]).
/// `stdio` entries are never dropped — that transport is baseline ACP v1,
/// no capability gate exists for it.
///
...
/// declared support for (`agentCapabilities.mcpCapabilities`, #1462) —
/// `stdio` needs no such flag (baseline ACP v1); this is only `http`/
/// `sse`. Captured per-session so [`build_mcp_servers_wire`] can drop a
/// user-configured `http`/`sse` MCP server the live agent doesn't actually
/// support, rather than sending it and letting `session/new` fail or
/// silently ignore it.
///
/// An agent that omits `mcpCapabilities` entirely (every pre-#1462 agent,
...
mod tests {
    ...
    fn parse_mcp_capabilities_reads_declared_flags_and_defaults_absent_to_false() {
        ...
        // Every pre-#1462 fixture/agent omits the field entirely.
