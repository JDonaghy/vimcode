...
**Backend-neutral is not the bar — quadraui-owned is the bar.** The rule above names `src/gtk/` and `src/tui_main/` and points shared logic at `render.rs`. That is still right for *vimcode's own* decisions: vim semantics, buffer and window state, which primitive to compose where. It is **not** a licence to implement a primitive's internals in `render.rs` because both backends will call it. If the code decides *how a primitive looks* — row/line sampling or downscaling, colour or coverage aggregation, dithering, glyph or cell packing, scroll/pitch geometry, a primitive's own hit-test math — it belongs in quadraui **even though it is backend-neutral**, via the same four steps.

**This is a blocking review rule.** A diff adding a new function to `render.rs` that does row/column/pixel/cell arithmetic is a `request-changes` **unless the PR states explicitly why no quadraui primitive can own it.** Same shape as the black-box-coverage bar: the exemption is real, but it must be claimed out loud, not assumed. `src/render.rs` is 20,000+ lines — it is the one place a primitive-sized algorithm can land with nobody noticing.

**Negative example (#1085, merged 2026-09-17).** The minimap point-sampled 1 buffer line in 5. The fix added ~130 lines to `render.rs` — `minimap_block_bounds`, `minimap_block_sample_indices`, `minimap_block_text`, `minimap_block_dither_threshold_met` — block partitioning, per-column coverage aggregation, and an ordered (Bayer) dither. Every gate passed it: zero `src/gtk/` or `src/tui_main/` lines, no OS assumptions, an excellent RED-verified `TuiDriver` test. It was wrong anyway. `quadraui::primitives::minimap`'s module doc states that row down-sampling lives in the primitive "so the two rasterisers never re-derive or re-reduce data the primitive already resolved" — so `build_minimap_data` now calls `quadraui::sample_lines(&borrowed, borrowed.len())`, deliberately the no-op path, because the host already did the primitive's job. quadraui grew a second dither at a different level (quadraui#1007) the same week, and the host copy regressed the #728 perf guard (#1096). Being lifted back upstream by quadraui#1012 / #1098.

**An issue or briefing does not get to waive this.** #1085's own body asserted "This is the host's decision, not quadraui's", and the worker and reviewer both correctly deferred to it — which is exactly how the rule was bypassed without anyone breaking it. If a briefing rules quadraui out of scope without saying *why the primitive cannot own it*, that assertion is the thing to push back on (see "Push back actively" above), not settled scope.
