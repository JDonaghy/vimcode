# src/win_gui/draw.rs — Win-GUI Rendering (4,614 lines)

Direct2D rendering of `ScreenLayout`. Consumes platform-agnostic layout and paints via D2D render target + DirectWrite.

## Key Types
- `DrawContext` — rt, dwrite, format, ui_format, icon_format, theme, char_width, line_height, editor_left, nerd_icon_font
...
- `draw_editor_window(rw)` — clipped editor: gutter (unscrolled), text content with scroll_left offset (scrolled), horizontal scrollbar, vertical scrollbar, fold markers
- `draw_sidebar(sidebar, screen, engine)` — activity bar icons (Nerd Font when available, Segoe MDL2 fallback), panel background, panel content dispatch
