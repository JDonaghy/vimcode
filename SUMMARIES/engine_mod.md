# src/core/engine/mod.rs — 3,342 lines
...
- `PickerSource` / `PickerItem` / `PickerAction` — unified picker types
- `Dialog` / `DialogButton` / `DialogInput` — modal dialog system
- `PaletteCommand` — command palette entry
...
- `EditorGroup` — tab group with own tab list
