...
- `handle_explorer_msg()` — file tree open/preview/create/delete/refresh/focus/activate-selected; routes `ExplorerKey/RightClick/ExplorerUiEvent` + `PromptRenameFile/NewFile/NewFolder`. `ExplorerUiEvent` routes mouse events through `TreeController.handle()` for scrollbar interaction (#415)
...
- `handle_explorer_da_key/click/right_click` — DA input handlers (click handler no longer has scrollbar hit-test — handled by TreeController #415)
