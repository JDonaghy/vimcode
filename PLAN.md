...
## Recently Completed
- **Session 268**: Win-GUI bug fixes (16 items). Original 9 critical+medium fixes, plus: tabs disappear with 2+ groups (breadcrumb offset in draw + click + drag overlay), terminal steals focus, tab accent only on active group, sidebar focus persistence (systematic audit found 2 missing `clear_sidebar_focus()` calls), dialog overflow (auto-sized width). Created `docs/NATIVE_GUI_LESSONS.md` with 9 sections of lessons for future backends. New test: `test_multi_group_window_rects_cover_all_groups`.
