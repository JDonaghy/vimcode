...
- [ ] **Native Windows GUI (Direct2D + DirectWrite)** — Replace GTK4 on Windows with a native Win32 backend using `windows-rs` + Direct2D rendering + DirectWrite text. Produces `vimcode-win.exe` binary via `win-gui` Cargo feature. Architecture: new `src/win_gui/` directory (~12-18K lines) consuming the same `ScreenLayout` from `render.rs` — zero changes to core engine. DirectWrite provides ClearType subpixel AA (non-negotiable for a code editor). Phase 1: Win32 HWND + D2D render target + DWrite text + keyboard input → single editor pane with syntax highlighting. Phase 2: Tab bar, mouse, completion/hover popups, clipboard. Phase 3: File explorer, native dialogs (IFileOpenDialog), terminal panel, sidebars. Phase 4: Multi-monitor DPI, IME composition, file watching, accessibility. Key files: `src/win_gui/mod.rs` (HWND + message loop), `draw.rs` (D2D rendering), `click.rs` (hit-testing), `input.rs` (WM_KEYDOWN/WM_CHAR/WM_IME → engine key names), `dialogs.rs` (native file dialogs), `util.rs` (DPI, COM, color conversion).
...
- [ ] **Multi-backend prep work** — Extract shared hit-testing geometry, key binding matching, and open-url logic from GTK/TUI backends into `render.rs` and `core/` so all backends (GTK, TUI, Win-GUI, macOS) reuse the same code. `Color::to_f32_rgba()` for D2D/CoreGraphics. `view_row_to_buf_line()`/`view_row_to_buf_pos_wrap()` in render.rs. `open_url_in_browser()` in core/engine. Consider `notify` crate for cross-platform file watching (replaces GTK's gio::FileMonitor).
