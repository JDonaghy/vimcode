...
## Recently Completed
- **Session 278**: Find/replace hit regions + shared dispatch + visual selection preservation — `FindReplaceClickTarget` enum, `FrHitRegion` struct, `compute_find_replace_hit_regions()`, `handle_find_replace_click()`, `find_word_boundaries()`. TUI/GTK click handlers rewritten with hit regions (fixes ≡/× button misalignment). Win-GUI migrated to shared dispatch. Drag-to-select + double-click word select for TUI. Visual selection stays visible during Ctrl+F (frozen via `find_replace_visual_end`). Dynamic panel width for long match counts. 14 new tests. Crate extraction roadmap (7 items).
