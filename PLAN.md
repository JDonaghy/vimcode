...
- [ ] **Win-GUI: load bundled Nerd Font via DirectWrite** — The bundled `data/fonts/vimcode-icons.ttf` (13KB Nerd Font subset) is currently only usable by the GTK backend (installed to `~/.local/share/fonts/`). Win-GUI can't use it because `CreateTextFormat` only sees system-installed fonts. Fix: use `IDWriteFactory::CreateCustomFontCollection` or `IDWriteFontSetBuilder` to load the TTF from embedded bytes (`include_bytes!`) into a custom font collection at startup. Create a dedicated `IDWriteTextFormat` for the Nerd Font, used by activity bar icons, file explorer icons, and ext panel icons. This would eliminate the Segoe MDL2 icon mapping and make all `icons::` Nerd Font glyphs render natively, matching GTK. The TUI backend would still depend on the user's terminal font (no fix possible there — terminal controls rendering).
