...
- [~] **Debugger (DAP)** — Transport + adapter registry + `:DapInstall` (S83); poll loop + breakpoint gutter `●`/`◉` + stopped-line highlight + Rust binary launch fix (S84). Remaining: variables/call-stack panel (S85), DAP output console, inline variable virtual text
