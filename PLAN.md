...
## Recently Completed
- **Session 245**: Editor action menu, richer tree-sitter highlighting, explorer color overhaul. Action menu: `⋯` button with 8-item dropdown, both GTK+TUI. Tree-sitter: 12 new Theme fields (`control_flow`/`operator`/`punctuation`/`macro_call`/`attribute`/`lifetime`/`constant`/`escape`/`boolean`/`property`/`parameter`/`module`), all 20 language queries expanded, keywords split into storage vs control flow, `semantic_token_style()` handles `controlFlow` modifier, fixed reparse (always full parse — no `tree.edit()` meant stale byte offsets), insert mode immediate re-parse. Explorer: same base color for files/dirs, recursive git/diagnostic propagation, GTK indicator in own column. Fixes: split-down icon, `gtk_editor_bottom()` shared helper, divider drag excluded from tab bars, GTK menu dropdown padding, LSP status no longer downgrades on empty semantic tokens. 7 new tests (5282 total).
...
- [ ] **Externalize highlight queries into extensions** — Move tree-sitter highlight queries from hardcoded strings in `syntax.rs` into extension manifests (e.g. a `highlights.scm` field or bundled `.scm` file). Each language extension would define its own queries, decoupling language-specific knowledge from the core. This also enables user customization of highlight patterns without recompiling.
