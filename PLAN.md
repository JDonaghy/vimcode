...
- [ ] **Settings editor** — `:Settings` command opens a dedicated settings panel. GTK: form generated from the `Settings` struct (each field = one row with label, type-appropriate input widget, and default value indicator); validated before saving; live preview where applicable. TUI: shows the raw `settings.json` content in an editable buffer. The `Settings` struct must include default keybindings for every editor command (as a `keybindings: HashMap<String, String>` or per-section sub-structs). `settings.json` is always created on first launch with all defaults (already implemented via `Settings::ensure_exists()`). IMPORTANT: the settings struct must be updated whenever new user-configurable options are added. History (command/search) lives in a separate `history.json` (already moved).
