...
## Recently Completed
- **Session 244**: TUI rendering artifact mitigation — `terminal.clear()` on resize + popup dismiss; debug panel close bug removed from BUGS.md.
- **Session 243**: LSP status indicator — `LspStatus` enum (`None/Installing/Initializing(name)/Running(name)/Crashed`) in `lsp_manager.rs`; `server_has_responded` HashMap tracks first meaningful response (hover with content, definition with locations); `lsp_status_for_buffer()` on Engine combines installing + manager state; render-side override: `Running` downgraded to `Initializing` when `BufferState.semantic_tokens` is empty (semantic tokens arrive after full indexing, aligning with hover/definition readiness); status bar shows server binary name (`rust-analyzer`), `…` suffix while indexing, `✗` on crash; `StatusAction::LspInfo` click runs `:LspInfo`; 1 new test (5275 total).
- **Session 242**: Clickable status bar segments + line endings — `StatusAction` enum (`GoToLine/ChangeLanguage/ChangeIndentation/ChangeLineEnding/ChangeEncoding/SwitchBranch`) on `StatusSegment`; `handle_status_action()` on Engine; `PickerSource::Languages` (37 languages from `all_known_language_ids()`), `PickerSource::Indentation` (6 presets), `PickerSource::LineEndings` (LF/CRLF picker); `LineEnding` enum with auto-detection on file open, `set_line_ending()` rope converter, preserved on save/reload; `SyntaxLanguage::from_language_id()` + `Syntax::new_from_language_id()` for language switching; new status segments: indentation (`Spaces: 4` / `Tab Size: 4`), line ending (`LF` / `CRLF`), git branch (clickable); TUI `status_segment_hit_test()` + GTK `gtk_status_segment_hit_test()` with `ClickTarget::StatusBarAction`; fixed TUI global status bar guard consuming per-window clicks; fixed GTK `pixel_to_click_target` second `editor_bottom` not accounting for per-window status or bottom panels; 4 new tests (5273 total).
- **Session 241**: Per-window status lines (Vim-style) — `StatusSegment`/`WindowStatusLine` structs; `build_window_status_line()` derives bar colors from `theme.background.lighten(0.10)` (no hardcoded hex); active window shows bold mode/filename/branch/filetype/encoding/Ln:Col, inactive shows dimmed filename/Ln:Col; mode text tinted for Insert (green), Visual (purple), Replace (red); `window_status_line` setting (default true, `:set windowstatusline`/`:set nowindowstatusline`); global status bar removed when per-window active; TUI `render_window_status_line()` + GTK `draw_window_status_bar()`; horizontal separator suppression; per-window click consumption; 6 theme color fields (`status_mode_*_bg`, `status_inactive_bg/fg`); 6 new tests (5265 total).
- **Session 240**: Cursorline highlight (default on, `cursorline_bg` derived from theme background via `Color::cursorline_tint()`), breadcrumb picker pre-selects current function, GTK tab bar padding (1.6× line_height, 14px horizontal pad, wider Command Center search bar 280px min), hardcoded colors audit (59 instances filed in BUGS.md), tree-sitter highlight query expansion added to roadmap.
- **Session 239**: Tree-style symbol drill-down — added `hierarchicalDocumentSymbolSupport` LSP capability (root cause: server was returning flat `SymbolInformation` instead of hierarchical `DocumentSymbol`); `parse_document_symbols_hierarchical` preserves children; `rebuild_tree_from_containers` reconstructs tree from flat `container` field; `PickerItem` gains `depth`/`expandable`/`expanded`; `SymbolKind::sort_order()`; Enter/Right/Left expand/collapse; click-to-toggle-expand in GTK+TUI; `▼`/`▷` arrows + indentation; picker jumps center viewport via `scroll_cursor_center()`; `breadcrumb_scoped_parent` cleared in `open_picker()`; 14 new tests (5080 total).
...
> Sessions 233 and earlier in **SESSION_HISTORY.md**.
