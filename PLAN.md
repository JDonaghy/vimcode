...
- [ ] **Win-GUI: git panel rendering parity** — The built-in Source Control (git) panel has keyboard routing (Session 270) but the renderer is a basic stub. Missing vs TUI/GTK: selection highlight on the focused item, commit input box (multi-line text entry with cursor), action toolbar buttons (commit/push/pull/sync/refresh), scrollbar, help popup (`?`), branch picker rendering, merge conflict indicators. The `draw_git_panel()` function currently renders branch info + staged/unstaged sections with status chars, but no interactivity feedback. Click handling for individual items (stage/unstage on click) is also missing — keyboard `s`/`d` works but mouse doesn't select items.
