...
- ~~**Unified find/replace overlay (Ctrl+F)**~~ **Done** — Engine-level `FindReplacePanel` rendered by all 3 backends. Replaces GTK-only Revealer dialog. Incremental search, replace/replace-all, case/whole-word/regex toggles, match count, mouse click handling. `ctrl_f_action` setting. 11 new tests.
...
## Recently Completed
- **Session 279**: Vim conformance matrix tests (Phase 1) — 29 tests/93 cases covering d/c/y/>/</ gU/gu/g~/count/dot-repeat × motions. `send_keys` + `setup_engine` test infrastructure. 4 bug fixes: `ge` motion, leader key intercepting `df<space>`, `dw` dot-repeat, `send_keys` `<<` parsing. `:set` option audit (Phase 2) — 18 tests covering round-trip, behavior, and ex-command verification. 6 Vim deviations documented as TODOs. 47 new tests.
- **Session 278**: Find/replace hit regions + colorcolumn + `x`+`.` fix — `FindReplaceClickTarget` enum, shared `handle_find_replace_click()`, `find_word_boundaries()`. TUI/GTK click handlers rewritten with hit regions. Visual selection preserved during Ctrl+F. Dynamic panel width. `:set colorcolumn` implemented (parsing, theme color derived from `background.colorcolumn_tint()`, 3-backend rendering). `x` count+`.` repeat fixed. CLAUDE.md rules elevated (theme colors, testing, hit regions). Vim conformance + crate extraction roadmap items. 27 new tests.
- **Session 277**: Visual-mode `:` with `'<,'>` range prefix — `command_from_visual` tracks originating visual mode, selection stays visible during command input, status line shows COMMAND, `mode_str()` updated. 6 new tests.
- **Session 276**: Unified find/replace overlay — engine-level `FindReplacePanel` with VSCode layout, rendered by all 3 backends. Replaces GTK Revealer dialog. Full feature set: incremental search, replace/all, case/whole-word/regex/preserve-case/in-selection toggles, nav buttons, close button, chevron expand, regex multiline mode, Ctrl+Z undo passthrough, Ctrl+A select-all in inputs, visual selection pre-fill, Nerd Font icons, Edit menu integration. Win-GUI mouse interactions (drag-select, double-click word, cached hit-rect). TUI/GTK parity gaps logged. 13 new tests.
- **Session 275**: Win-GUI horizontal scrollbar drag (draw + click + drag + scroll_left text offset + text-area clip). Bundled Nerd Font via DirectWrite (per-user font install + registry + icon_text_format). Phase 2c source-code verification test (grep Win-GUI source for 26 required engine method calls). 1 new test.
- **Session 274**: Phase 2d behavioral backend parity tests — 16 end-to-end tests simulating user interaction sequences (tab click/close, context menus, double-click, hover lifecycle, sidebar focus, terminal ops, tab drag-drop, preview promotion). Clippy CI fix (`needless_return` in `lsp_manager.rs`).
- **Session 273**: Windows LSP fix — `path_to_uri` produced backslash URIs breaking all LSP on Windows. Extension install fixes: Win-GUI `pending_terminal_command`, PowerShell terminal wrapper, `&&`→`;`, rustup proxy detection (`cargo_bin_probe_ok`), rust-analyzer install via `rustup component add`. Win-GUI hover: added `editor_hover_mouse_move` + `poll_editor_hover`. Install spinner clear fix.
- **Session 272**: Win-GUI git panel rendering parity — full `draw_git_panel()` rewrite with commit input, buttons, 4 collapsible sections, selection highlight, scrollbar, branch picker popup, help dialog. Click interactivity: section items, commit input, buttons, double-click-to-open-diff. Hover dwell for commit log popups. Tab bar scroll-into-view fix (`set_tab_visible_count` reporting).
- **Session 271**: Win-GUI extension panels (full `draw_ext_panel`, activity bar icons, click/keyboard/scroll handlers, `ext_panel_focus_pending` polling). Breadcrumb/tooltip UNC prefix fix. Empty breadcrumb covering tab bar fix. Diff toolbar tab overlap fix. Nerd Font auto-detection on Windows (`detect_nerd_font_windows()`). TUI hardcoded Nerd Font icons replaced with `icons::` fallback-aware calls. Default `use_nerd_fonts=false` on Windows.
- Sessions 265-270 archived in **SESSION_HISTORY.md**.
...
- [x] **Visual-mode `:` command with selection range** — Pressing `:` in Visual mode enters Command mode with `'<,'>` range prefix. `command_from_visual: Option<Mode>` on Engine tracks originating visual mode; `build_selection()` renders highlight during Command mode; selection cleared on Escape/Enter. Status line shows COMMAND. Works with `:s/` and `:norm`. 6 new tests.

- [x] **Find/replace: centralize hit-test geometry and port TUI/GTK mouse interactions** — **Done** — `FindReplaceClickTarget` enum + `FrHitRegion` struct + `compute_find_replace_hit_regions()` in `engine/mod.rs`; shared `handle_find_replace_click()` dispatch; `find_word_boundaries()` utility. TUI: rewritten click handler uses hit regions from `ScreenLayout`, added `fr_input_dragging` for drag-to-select, double-click word select. GTK: rewritten click handler uses hit regions (fixes `info_w` 60→80 mismatch, `popup_y` mismatch, toggle width mismatch). Win-GUI: migrated to shared dispatch. 14 new tests. Establishes pattern for crate extraction: render layer computes hit regions, backends only translate coords + call shared dispatch.
...
- [x] **Phase 2d: Behavioral backend parity tests (Option B)** — 16 end-to-end tests that simulate user interaction sequences and assert engine state transitions are correct. Covers: tab click/switch, tab close (clean + dirty gate), context menu lifecycle (open/confirm/dismiss), explorer/tab/editor context menu targets, double-click word selection, editor hover lifecycle (show/focus/scroll/dismiss), sidebar focus toggle + clear, terminal new/close/split, tab drag-drop to create splits, preview tab promotion via goto_tab, preview reuse invariant, mouse click cursor movement. Each test calls the same engine methods backends must call, catching method-call sequence bugs. Builds on the `UiAction` contract from Phase 2c.
...
- [x] **Win-GUI: git panel rendering parity** — Full `draw_git_panel()` rewrite: header with branch + ahead/behind, commit input box (multi-line + cursor), button row (Commit/Push/Pull/Sync with focus/hover), 4 collapsible sections (Staged/Changes/Worktrees/Log), selection highlight, file status coloring, path truncation, scrollbar, branch picker popup (search + create), help dialog. Click handling: section items, commit input, buttons, double-click-to-open-diff. Hover dwell for commit log popups. Button hover tracking on mouse move.
- [x] **Win-GUI missing features (from GTK comparison)** — All features implemented. Horizontal scrollbar drag added in Session 275 (draw + click + drag + scroll_left text offset + text-area clip). (Breadcrumb clicks, group divider drag, diff toolbar buttons, diff peek keys, tab tooltip dismiss fixed in Session 269.)
...
- [x] **`:set` option audit** — 18 tests: round-trip (33 settings), behavior (expand_tab, shift_width, ignorecase, smartcase, scrolloff, splitbelow/right, auto_pairs, hlsearch), ex-command (toggle, key=val, query, error).
- [x] **Operator × motion matrix** — 29 tests / 93 cases covering d/c/y/>/</ gU/gu/g~ × w/b/e/W/B/E/0/$/{/}/gg/G/f/t/F/T/% + text objects + count + `.` repeat. Found & fixed 4 bugs (ge motion, leader+space, dw dot-repeat, send_keys parsing). 6 Vim deviations documented.
- [ ] **Phase 3: `vimcode.feedkeys()` Lua API** — Add Lua plugin API functions that enable running Neovim's test suite directly against VimCode. This unlocks Phase 4 AND makes the plugin API much more powerful. Implementation:
  - `vimcode.feedkeys(keys)` — inject keystrokes into the engine (calls `handle_key()` for each parsed key, same notation as `send_keys` test helper: `"d2w"`, `"<Esc>"`, `"<C-a>"`)
  - `vimcode.eval(expr)` — evaluate Vim-like expressions (`@a` for registers, `&tabstop` for options, `line('.')`, `col('.')`)
  - `vimcode.buf.get_lines(start, end)` — range-based line access (0-indexed, Neovim-compatible)
  - `vimcode.buf.set_lines(start, end, lines)` — range-based line replacement
  - Key files: `src/core/engine/plugins.rs` (Lua function registration), `src/core/engine/keys.rs` (`handle_key()` entry point)
  - **Current Lua API status**: Has `vimcode.buf.lines()`, `vimcode.buf.line(n)`, `vimcode.buf.set_line()`, `vimcode.buf.cursor()`, `vimcode.buf.set_cursor()`, `vimcode.state.mode()`, `vimcode.command_run()`, `vimcode.keymap()`, `vimcode.state.register()`. Missing: `feedkeys`, `eval`, range-based `get_lines`/`set_lines`.
  - **Neovim API equivalents needed** (12 core functions): `nvim_feedkeys`→`vimcode.feedkeys`, `nvim_buf_get_lines`→`vimcode.buf.get_lines`, `nvim_buf_set_lines`→`vimcode.buf.set_lines`, `nvim_win_get_cursor`→already exists, `nvim_win_set_cursor`→already exists, `nvim_get_mode`→already exists, `nvim_exec2`→`vimcode.command_run`, `nvim_get_current_line`→`vimcode.buf.line(cursor_line)`, `nvim_eval`→`vimcode.eval`, `nvim_set_var`→not needed initially, `nvim_replace_termcodes`→handled by feedkeys parser, `nvim_set_keymap`→already exists.
- [ ] **Phase 4: Neovim test mining** — Two paths depending on whether Phase 3 is complete:
  - **Path A (after Phase 3):** Write a busted-compatible Lua shim that maps `helpers.feed()` → `vimcode.feedkeys()`, `helpers.expect()` → buffer content assertion, `helpers.curbuf_contents()` → `table.concat(vimcode.buf.lines(), '\n')`, etc. Run Neovim's Lua tests in VimCode's Lua sandbox with minimal edits. Test runner: either busted-compatible or a custom `vimcode.test()` wrapper.
  - **Path B (without Phase 3):** Manually translate Neovim test cases to Rust using the `send_keys` + `assert_eq` infrastructure from Phase 1. Slower but no dependency on Lua API work. Batch 10-20 test translations per session.
  - **Priority Neovim test files:** `test/old/testdir/test_normal.vim` (operator/motion edge cases: empty lines, EOF, single-char lines), `test_textobjects.vim` (nested quotes, mismatched brackets), `test_visual.vim` (visual mode operators), `test_undo.vim` (undo tree branching), `test/functional/legacy/normal_cmd_spec.lua` (Lua equivalents of the above).
  - **Sources:** `github.com/neovim/neovim/test/old/testdir/` (Vim's original test suite), `github.com/neovim/neovim/test/functional/legacy/` (Lua-based equivalents using busted + helpers).
- [ ] **Phase 5: `:help` coverage audit** — Walk Vim's documentation section by section, tag each feature as: implemented, partially implemented, not implemented, or intentionally skipped. Produce a coverage report. Focus on Normal mode commands, ex commands, and options. Lower priority until operator matrix is solid — bugs found here tend to be "missing feature" not "wrong behavior".
- [ ] **Phase 6: Automated buffer fuzzing** — Generate random key sequences, run in both Neovim headless (`nvim-rs` crate for msgpack RPC) and VimCode engine, compare buffer content and cursor position. On divergence, minimize the key sequence and output a failing test case. Put behind `#[cfg(feature = "fuzz")]` gate with separate `cargo test --features fuzz` invocation. Architecture: (1) Rust test binary generates weighted random key sequences (favor operators/motions/counts), (2) same sequence sent to Neovim via `nvim --headless --embed` + msgpack RPC, (3) after N keystrokes compare buffer + cursor, (4) on divergence minimize and emit test case. Longest lead time — start with the harness (connect to Neovim, send keys, read buffer) before adding randomization.

#### Vim Conformance Known Deviations (found in Session 279, documented as TODOs in tests)
- `d}` / `d{` — paragraph motion boundary differs from Vim (VimCode deletes the blank separator line)
- `da"` / `da'` — "around" quote text objects don't include trailing whitespace like Vim
- `di<` / `da<` — angle bracket text objects not yet implemented (missing from `find_text_object_range`)
- `2d2w` — count×count multiplication not working (VimCode concatenates digits: `2d2w` → `d22w`)
- `<G` — outdent operator with `G` motion doesn't work (`>G` indent works fine)
- Cursor position after `c`+`<Esc>` — VimCode doesn't always move cursor one left on Esc like Neovim does
...
### Crate Extraction (vimcode-core)
- [ ] **Hit regions for tab bar** — Add `Vec<(HitRegion, TabBarClickTarget)>` to `ScreenLayout` group tab bars. Migrate tab click/close/drag targets from per-backend geometry to shared regions. Retire duplicated tab slot geometry in GTK/TUI/Win-GUI click handlers.
- [ ] **Hit regions for status bar** — Add hit regions to `WindowStatusLine` in render.rs. Migrate `StatusAction` dispatch (already uses shared enum, just needs regions). Covers Ln:Col, filetype, indent, branch, layout toggles.
- [ ] **Hit regions for sidebar** — Activity bar icons (fixed positions), explorer tree rows, git/search/debug panel items. Needs scroll-aware hit region system (positions relative to scroll offset).
- [ ] **Hit regions for context menus + dialogs** — Context menu items already engine-driven (`ContextMenuState.items`). Compute hit regions from item count × row height. Dialog buttons similarly.
- [ ] **Abstract event layer** — `UiEvent` enum: `Click(HitTarget)`, `Drag(from, to)`, `DoubleClick(HitTarget)`, `Scroll(target, delta)`, `KeyPress(key)`. Backends translate native events → `UiEvent`, engine processes them. Final step before crate extraction.
- [ ] **Extract `vimcode-core` crate** — `core/` + `render.rs` become standalone crate. Public API: `Engine::new()`, `Engine::handle_event(UiEvent)`, `Engine::build_screen_layout() -> ScreenLayout`. Backends become thin adapters.
- [ ] **Proof-of-concept SQL client** — New binary using `vimcode-core` for TUI rendering + hit regions. Custom "SqlEngine" with query editor (reusing Buffer/View), results table, connection sidebar. Validates crate API generality.
