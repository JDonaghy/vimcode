...
## Recently Completed
- **Session 268**: Win-GUI critical + medium bug fixes (7 total). Tab close dirty check, picker mouse interaction, dialog button clicks, QuitWithUnsaved confirmation dialog + WM_CLOSE handler, fold-aware scrolling, picker scroll interception, VSCode selection clear on click. Added `close_tab_confirm` and `quit_unsaved` dialog tags to engine's `process_dialog_result`.
...
- [x] **Win-GUI critical bug fixes (from GTK comparison)** — 4 critical + 3 medium bugs fixed: (1) tab close dirty check via engine dialog, (2) picker mouse interaction (click-to-select, click-outside-dismiss, scroll navigation), (3) dialog button click handling with rect hit-testing, (4) QuitWithUnsaved shows confirmation dialog + WM_CLOSE handler, (5) fold-aware scrolling via `scroll_down_visible`/`scroll_up_visible`, (6) picker scroll interception, (7) VSCode selection clear on click. Remaining medium: cursor viewport tracking after scroll, terminal tab switching.
- [ ] **Win-GUI missing features (from GTK comparison)** — 4 low-priority features missing vs GTK: breadcrumb clicks, group divider drag, horizontal scrollbar drag, diff toolbar button clicks, diff peek key routing, tab tooltip dismiss-on-mouseout.
