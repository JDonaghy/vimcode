...
- **Session 270**: Win-GUI bug blitz (9 fixes). Subprocess hiding (`hidden_command()` helper, 6 curl call sites), full NCHITTEST for all 8 resize zones, keyboard routing for Search/AI/Git panels (`on_key_down` + `on_char`), nav arrow click handlers, panel-specific scroll wheel routing, activity bar focus flag fixes for Search/Debug.
...
- [ ] **Visual-mode `:` command with selection range** — In Neovim, pressing `:` in Visual mode enters Command mode with `'<,'>` range prefix, allowing `:'<,'>s/find/replace/g` to substitute only within the selection. VimCode has `Mode::Command` but doesn't auto-populate the `'<,'>` range when entering from Visual mode. The find/replace overlay should also integrate with this: Ctrl+F from Visual mode should behave like a GUI version of `:'<,'>s/` — restricting search/replace to the selection. This is partially implemented via the `in_selection` toggle and `find_replace_selection_range`, but needs the `:` command pathway to also support it natively.

- [ ] **Find/replace: centralize hit-test geometry and port TUI/GTK mouse interactions** — The Win-GUI find/replace dialog has working drag-to-select, double-click word select, cached `FindReplaceRect` for accurate click handling, and selection rendering. The TUI and GTK backends are missing these features and compute hit-test coordinates independently from draw code, causing geometry mismatches (≡ button closes dialog, × button invisible, etc). **Approach:** (1) Move hit-test geometry into the `FindReplacePanel` render struct — compute all button positions during `build_screen_layout()` so they're available to every backend without recomputation. (2) Port Win-GUI's `fr_input_dragging`, double-click word select, and `FindReplaceRect` cached-position approach to TUI and GTK. (3) Systematic audit: verify every click target works in all 3 backends using the same test sequence (open, type, click toggles, click ≡, click ×, drag-select in input, double-click word, Ctrl+A, replace, replace all).
